##########################################################################
# Project:      Step 2 - Simulations
#               
# Author:       Sarah Seelye
#
# Date Updated: 2022 Dec 09
##########################################################################
...
happi<-read_dta("data/kp_va_happi_aim3_20221103.dta") #1560126 obs, 183 vars

'%notin%'<-Negate('%in%')

#Exclude hospitalizations at hospitals with fewer than 300 hospitalizations 
happi<-happi[happi$hospid %notin% c('542','523A5', '657A0', '598A0', '509A0', '561A4', '610', '537BU'),] #1559523
...
write.csv(table, "Tables/table with raw rates by subgroup_20221209.csv", row.names=TRUE)
...
write.csv(out, "Tables/subgroup_rates_20221209.csv")
...
sepsis<-as.data.frame(cbind(uniqid=sepsis$uniqid, pmort30_orig=sepsis$pmort30_orig, pmort30_fast=sepsis$pmort30_fast))

#Expected number of deaths
mean(sepsis$pmort30_orig)*nrow(sepsis) #n=44862
sum(sepsis$pmort30_orig)
...
sim<-function(hospital.type){ 
  ...
  new.time.sepsis<-median(check$tta_fast, na.rm = TRUE)
  ...
  new.time<-median(sample$tta_fast, na.rm = TRUE)
  ...
  new.mort30<-mean(sepsis.sample$pmort30_fast)*nrow(sepsis.sample)
  old.mort30<-mean(sepsis.sample$pmort30_orig)*nrow(sepsis.sample)
  
  #Expected probability of sepsis death before and after increasing the time to abx
  new.mort30prob<-mean(sepsis.sample$pmort30_fast)*100
  old.mort30prob<-mean(sepsis.sample$pmort30_orig)*100
...
write.csv(sim.high.results, "Tables/sim.high.results_20221209.csv", row.names=TRUE)
write.csv(sim.high.outcomes, "Tables/sim.high.outcomes_20221209.csv", row.names=TRUE)
...
write.csv(sim.middle.results, "Tables/sim.middle.results_20221209.csv", row.names=TRUE)
write.csv(sim.middle.outcomes, "Tables/sim.middle.outcomes_20221209.csv", row.names=TRUE)
...
write.csv(sim.low.results, "Tables/sim.low.results_20221209.csv", row.names=TRUE)
write.csv(sim.low.outcomes, "Tables/sim.low.outcomes_20221209.csv", row.names=TRUE)
...
write.csv(out1, "Tables/Simulation results of high_20221209.csv", row.names=TRUE)
write.csv(out2, "Tables/Simulation results of middle_20221209.csv", row.names=TRUE)
write.csv(out3, "Tables/Simulation results of low_20221209.csv", row.names=TRUE)
...
write.csv(results, "Tables/Simulation results of abx outcomes_20221209.csv", row.names=FALSE)
