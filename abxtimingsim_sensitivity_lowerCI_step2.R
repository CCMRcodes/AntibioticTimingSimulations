...
happi<-read_dta("kp_va_happi_aim3_lowerCI.dta") #1559523 obs, 184 vars
...
#write.csv(table, "table with raw rates by subgroup_lowerCI.csv", row.names=TRUE)
...
#write.csv(out, "subgroup_rates_lowerCI.csv")
...
sepsis<-as.data.frame(cbind(uniqid=sepsis$uniqid, pmort30_orig_ll=sepsis$pmort30_orig_ll, pmort30_fast_ll=sepsis$pmort30_fast_ll))

#Expected number of deaths
mean(sepsis$pmort30_orig_ll)*nrow(sepsis) #n=44566
sum(sepsis$pmort30_orig_ll)
...
sim<-function(hospital.type){ 
  ...
  new.time.sepsis<-median(check$tta_fast_ll, na.rm = TRUE)
  ...
  new.time<-median(sample$tta_fast_ll, na.rm = TRUE)
  ...
  new.mort30<-mean(sepsis.sample$pmort30_fast_ll)*nrow(sepsis.sample)
  old.mort30<-mean(sepsis.sample$pmort30_orig_ll)*nrow(sepsis.sample)
  
  #Expected probability of sepsis death before and after increasing the time to abx
  new.mort30prob<-mean(sepsis.sample$pmort30_fast_ll)*100
  old.mort30prob<-mean(sepsis.sample$pmort30_orig_ll)*100
...
#write.csv(sim.high.results, "sim.high.results_lowerCI.csv", row.names=TRUE)
#write.csv(sim.high.outcomes, "sim.high.outcomes_lowerCI.csv", row.names=TRUE)
...
#write.csv(sim.middle.results, "sim.middle.results_lowerCI.csv", row.names=TRUE)
#write.csv(sim.middle.outcomes, "sim.middle.outcomes_lowerCI.csv", row.names=TRUE)
...
#write.csv(sim.low.results, "sim.low.results_lowerCI.csv", row.names=TRUE)
#write.csv(sim.low.outcomes, "sim.low.outcomes_lowerCI.csv", row.names=TRUE)
...
#write.csv(out1, "Simulation results of high_lowerCI.csv", row.names=TRUE)
#write.csv(out2, "Simulation results of middle_lowerCI.csv", row.names=TRUE)
#write.csv(out3, "Simulation results of low_lowerCI.csv", row.names=TRUE)
...
#write.csv(results, "Simulation results of abx outcomes_lowerCI.csv", row.names=FALSE)
